...
const DOrder = () => {
    ...
    useEffect(() => {
        const fetchDistributors = async () => {
            try {
                const response = await fetch('http://localhost:8000/distributor/getAllDistributors', {
                    headers: {
                ...
                if (response.ok) {
                    ...
                    console.error('Failed to fetch distributors:', data.error);
                }
            } catch (error) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch('http://localhost:8000/dorder/createOrder', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                        ...
                        >
                            ...
                            {distributor.map((distributor) => (
                                <option key={distributor.distributorId} value={distributor.distributorId}>
                                    {distributor.name} || {distributor.distributorId}
                                </option>
                            ))}
                        </select>
                    </div>
